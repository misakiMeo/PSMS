...
begin
	...
	for R in (select * from Customer_1)  LOOP
		if N = R.mobile THEN
			cid := R.customer_id;
		end if;
	end loop;
	...
	if cid=-1 THEN
		for R in (select * from Customer_2@site_2)  LOOP
			if N = R.mobile THEN
				cid := R.customer_id;
			end if;
		end loop;
	...
	DBMS_OUTPUT.PUT_LINE('-----------------------------------------------------------------------------');
	select price into nPrice from Pets_1 where pet_id=T;
	...
	if O = 1 then
		...
		insert into Invoice_1 (invoice_number, total_price, date_) values (inv_no, total, date_c);
		insert into Invoice_3 (invoice_number, customer_id, details, payment_method, order_type) values (inv_no, cid, 'Bird', 'Cash', 'In-Store'); 
	elsif O = 2 THEN
		insert into Invoice_2@site_2 (invoice_number, total_price, date_) values (inv_no, total, date_c);
		insert into Invoice_4@site_2 (invoice_number, customer_id, details, payment_method, order_type) values (inv_no, cid, 'Bird', 'Card', 'In-Store'); 
	elsif O = 3 THEN
		insert into Invoice_2@site_2 (invoice_number, total_price, date_) values (inv_no, total, date_c);
		insert into Invoice_4@site_2 (invoice_number, customer_id, details, payment_method, order_type) values (inv_no, cid, 'Bird', 'bKash', 'In-Store'); 
	else
