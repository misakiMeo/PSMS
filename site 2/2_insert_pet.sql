...
Begin
	...
	if T = 1 THEN
		--DBMS_OUTPUT.PUT_LINE('Bird @ site 1');
		insert into Pets_1@site_1 (type_, gender, mutation, age, price) values ('Bird', nGender, M, A, P); 
	elsif T = 2 THEN
		insert into Pets_2 (type_, gender, mutation, age, price) values ('Dog', nGender, M, A, P);	
	elsif T = 3 THEN
		insert into Pets_3 (type_, gender, mutation, age, price) values ('Cat', nGender, M, A, P);	
	else
		raise invalid_choice;
	end if;
...
select * from Pets_1@site_1 union (select * from Pets_2 union select * from Pets_3);
