...
Begin
	...
	if C = 1 THEN
		--DBMS_OUTPUT.PUT_LINE('Food @ site 1');
		insert into Accessories_1@site_1 (title, pet_type, category, price, stock) values (TI, pt, 'Food', P, S); 	
	elsif C = 2 THEN
		insert into Accessories_2 (title, pet_type, category, price, stock) values (TI, pt, 'Toys', P, S); 	
	elsif C = 3 THEN
		insert into Accessories_2 (title, pet_type, category, price, stock) values (TI, pt, 'Cages', P, S); 	
	elsif C = 4 THEN
		insert into Accessories_2 (title, pet_type, category, price, stock) values (TI, pt, 'Food Accessories', P, S); 	
	elsif C = 5 THEN
		insert into Accessories_2 (title, pet_type, category, price, stock) values (TI, pt, 'Other', P, S); 	
	else
		raise invalid_choice;
	end if;
...
select * from Accessories_1@site_1 union (select * from Accessories_2);
